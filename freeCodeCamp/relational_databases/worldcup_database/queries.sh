...
echo "$($PSQL "(SELECT name FROM games INNER JOIN teams ON teams.team_id = games.winner_id WHERE year = 2014 AND round = 'Eighth-Final') UNION (SELECT name FROM games INNER JOIN teams ON teams.team_id = games.opponent_id WHERE year = 2014 AND round = 'Eighth-Final') ORDER BY name")"
